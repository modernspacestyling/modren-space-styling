...
-- Admin account MUST be created manually via the admin signup flow.
-- We intentionally DO NOT seed a default password here for security reasons.
-- To create your first admin:
--   1. Call POST /api/create-admin (server-side, protected by ADMIN_BOOTSTRAP_TOKEN env var)
--   2. OR insert directly via Supabase SQL Editor using pgcrypto's crypt() with a strong password
-- Example (run MANUALLY with a unique password — do NOT commit passwords):
--   INSERT INTO agents (email, password_hash, name, role, status)
--   VALUES ('admin@modernspacestyling.com.au', crypt('<YOUR-STRONG-PASSWORD>', gen_salt('bf')), 'MSS Admin', 'admin', 'approved');
