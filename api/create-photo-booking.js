...
const PACKAGE_PRICES = {
    ...
    premium: 449,
    ultimate: 599,
    rental: 196.90, // $179 + GST = $196.90 inc GST
};
...
module.exports = async function handler(req, res) {
    ...
    // Fetch admin/partner numbers first so we can parallelise all SMS.
    ...
    const smsTasks = [];
    smsTasks.push(sendSms(sanitize(clientPhone, 20), clientSms, jobNumber, supabase));
    ...
    if (partnerNumbers?.length) {
        for (const partner of partnerNumbers) {
            if (partner.phone_number === adminPhone) continue; // avoid duplicate
            smsTasks.push(sendSms(partner.phone_number, adminSms, jobNumber, supabase));
        }
    }

    // Run all SMS in parallel, capped so API returns fast even if Twilio is slow.
    ...
    try {
        const invoicePayload = {
            ...
            booking: {
                ...
                clientEmail: sanitize(clientEmail, 100).toLowerCase(),
                clientPhone: sanitize(clientPhone, 20),
                agency: sanitize(agency || '', 100),
                address: sanitize(address, 300),
        ...
        console.log('[create-photo-booking] Invoice generated:', invoiceRes._json);
    } catch (invoiceErr) {
        console.error('[create-photo-booking] Invoice generation failed (non-blocking):', invoiceErr.message);
    }
    ...
    return res.status(200).json({
        ...
        jobNumber,
        estimatedPrice,
        message: 'Photography booking received. Invoice emailed. SMS confirmation sent.',
    });
};
