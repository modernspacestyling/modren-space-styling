...
// Use this on ANY user-provided value before injecting into innerHTML.
// ============================================================
...
// Short alias for template literals
...
async function seedDatabase() {
  ...
  // Admin user — password is randomly generated on first seed and logged to console ONCE.
  // Production uses server-side auth via Supabase; this is only for local/offline demo mode.
  const randomBytes = crypto.getRandomValues(new Uint8Array(16));
  const demoPassword = Array.from(randomBytes).map(b => b.toString(36)).join('').slice(0, 16);
  const passwordHash = await Crypto.hash(demoPassword);
  // eslint-disable-next-line no-console
  console.log('[MSS local demo] Admin email: admin@modernspacestyling.com.au — password (shown ONCE):', demoPassword);
  ...
  localStorage.setItem('mss_pricing_config', JSON.stringify({
    ...
    masterBedroomRate: 300,  // Per master bedroom
    bathroomRate: 150,       // Per bathroom
    livingRate: 250,         // Per living area (updated 2026-04-15)
    diningRate: 150,         // Per dining area
    alfrescoRate: 200,       // Alfresco area
    pantryRate: 100,         // Walk-in pantry
    hallwayRate: 0,          // Hallway — complimentary
